...
The vault carries four: the entry search hosted in the app bar (`vault.entries`), the group filter above the group tree (`vault.groups`, plain text by default, regex on request, a match keeps the group's ancestors visible and expands them), the group-scope search inside the scope menu (`vault.group-scope`), and the detail pane's "Filter attachments & fields" search (`vault.attachments`), which narrows the field containers (Username, Password, URL, Modified) and the attachment rows; an unparsable regex leaves the previous result standing. Beside the entry search the health chips (Breached, Weak, Reused, Healthy) filter the list by verdict and carry live counts; Breached shows no count because breach exposure needs an online check the vault does not make.
