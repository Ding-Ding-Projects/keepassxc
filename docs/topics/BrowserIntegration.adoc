...
On Windows, when browser integration and automatic extension setup are enabled but all native-messaging registrations are missing, KeePassXC repairs the Chrome-family, Firefox-family, and Edge host registrations on startup. This recovers profiles whose browser integration setting survived an application update while the per-browser registration did not.
