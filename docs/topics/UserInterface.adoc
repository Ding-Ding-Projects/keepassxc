...
=== Screenshot Security
By default, KeePassXC allows recordings and screenshots of the application window. To exclude the window from screen capture on Windows and macOS (for example during meetings), open the _View_ menu and untick _Allow Screen Capture_; the choice is remembered across restarts. The `--prevent-screencapture` and `--allow-screencapture` command line flags override the remembered setting for a single run.
...
Options:
  ...
  --prevent-screencapture      Exclude the application from screen recording and screenshots
