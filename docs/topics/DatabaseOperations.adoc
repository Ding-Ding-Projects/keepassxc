...
Opening a database file always starts at its password, key file, and hardware-key form. When Quick Unlock has previously been enrolled for that database, its separate _Unlock Database_ action initiates biometric authentication. KeePassXC never opens Windows Hello merely because a `.kdbx` file was selected.
...
To add TOTP to a database entry, you must first retrieve the secret string from the website or application you are authenticating to. Often this secret is accompanied with a QR code and can be copy/pasted below. When creating a new entry, select _Set up TOTP..._ directly in the entry form and paste the secret before saving the entry. For an existing entry, use the entry's _TOTP_ -> _Set up TOTP..._ menu. Example:
...
Each successful save is also preserved as an encrypted KDBX snapshot in an isolated local Git repository under KeePassXC's application-data folder, with one repository per database identity. A History row that records removed entries offers _Restore_ and imports only entries missing from the current database; existing entries are never overwritten. The restore itself is appended as a new revision, and the database must be saved to make the restored entries permanent. The repository is never created beside the user's database file and has no remote.
